function ShowSpringboardScreen(episodes, selectedEpisode, leftBread, rightBread, isAudioContent)
	...
	while true
		...
		if msg <> invalid
			if msg.isScreenClosed()
				...
				if (isAudioContent)
					print 'calling ShowAudioScreen'
                    ShowAudioScreen(episodes[selectedEpisode])
                else
                    ShowVideoScreen(episodes[selectedEpisode])
                end if
